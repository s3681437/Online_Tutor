...
class EachTutor extends Component {
    render() {
        const { _id, tutorName, tutorAge, tutorGender, tutorNationality,
            tutorSpecialty, tutorExperience, busyTime, dateCreated, tutorImage } = this.props.tutor;
                ...
                {this.props.tutor === {} ? <h3>Sorry</h3> :
                    <div className="card" style={tutorCard}>
                        ...
                        <div className="card-body">
                            ...
                            <button className="btn btn-dark" onClick={this.props.passTutor.bind(this, _id, busyTime)}>
                                <Link to={`/schedule/${localStorage.getItem('userID')}`} style={{ color: "white" }}>Choose a schedule</Link>
                            </button>
                        </div>
                    </div>}
            </div>
